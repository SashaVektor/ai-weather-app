const cleanData = (data: Root, city: string) => {
    ...
    const { temperature, windspeed, weathercode, time, winddirection } = current_weather

    const {
        temperature_2m, snowfall, rain,
        relativehumidity_2m, precipitation_probability, uv_index
    } = hourly
